...
<head>
    ...
    <title>Vettix - Edit Ranking</title>
...
<div class="d-flex align-items-center mb-0">
        <div>
            <h4 class="fw-bold text-dark mb-0">Edit Ranking</h4>
            <h4 class="text-muted small mb-0" style="font-size: 15px; height: 7px;">Ubah detail ranking</h4>
        </div>
            ...
            <div class="col-md-8">
                <div class="card form-card p-4">
                    <div class="d-flex align-items-center mb-4">
                        ...
                        <h4 class="fw-bold mb-0">Edit Ranking Peserta</h4>
                    </div>
                    ...
                    <form action="{{ route('rankings.update', $ranking->id) }}" method="POST">
                                ...
                                <select name="event_id" id="event_id" class="form-select" required>
                                    ...
                                    @foreach($events as $event)
                                        <option value="{{ $event->id }}" data-event-name="{{ $event->nama_event }}"
                                            {{ $ranking->event_id == $event->id ? 'selected' : '' }}>
                                ...
                                <select name="participant_id" id="participant_id" class="form-select" required>
                                    ...
                                    @foreach($participants as $participant)
                                        <option value="{{ $participant->id }}" data-event-id="{{ $participant->event_id }}"
                                            {{ $ranking->participant_id == $participant->id ? 'selected' : '' }}>
                                            {{ $participant->nama_peserta }} ({{ $participant->nim }})
                                        </option>
                                    @endforeach
                                </select>
                                <small class="text-muted" id="participant-hint">
                                    <i class="fas fa-info-circle"></i> Ganti event untuk memfilter peserta
                                </small>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" name="rank" class="form-control" min="1" value="{{ $ranking->rank }}" required>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" step="0.01" name="score" class="form-control" min="0" value="{{ $ranking->score }}" required>
                            </div>

                            <div class="col-md-4 mb-3">
                                <label class="form-label fw-bold">Pencapaian</label>
                                <input type="text" name="achievement" class="form-control" value="{{ $ranking->achievement }}" placeholder="Contoh: Juara 1">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="notes" class="form-control" rows="3" placeholder="Catatan tambahan (opsional)">{{ $ranking->notes }}</textarea>
                        </div>

                        <div class="d-flex gap-2">
                            <a href="{{ route('rankings.index') }}" class="btn btn-light w-50">Batal</a>
                            <button type="submit" class="btn btn-vettix w-50">
                                <i class="fa-solid fa-save"></i> Update Ranking
                            </button>
                        </div>
                    </form>
                </div>
